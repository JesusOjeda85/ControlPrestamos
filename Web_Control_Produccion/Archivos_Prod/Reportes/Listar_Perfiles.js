    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#lstreportes').tree({
                ...
                onClick: function (node) {
                    if (node.children.length <= 0) {    
                        var valores = node.strclave.split('@');
                        IR_PAGINA('Reportes.aspx', 'fkorg=' + fkorganismo + "&perfil=" + concepto + "&nomrep=" + node.text+"&Reporte="+node.nombre+"&Proceso="+node.strclave+"&CargaInicial="+valores[2]); 
                    }
                }
            });
        },
