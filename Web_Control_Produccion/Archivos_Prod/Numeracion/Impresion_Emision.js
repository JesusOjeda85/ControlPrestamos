...
$(document).ready(function () {
    ...
    $('#btnPagare').bind('click', function () { IMPRESION_PROCESOS('#btnPagare', 'Pagare'); });

    $('#btnCheque').bind('click', function () { IMPRESION_PROCESOS('#btnCheque', 'ChequeStd'); });

    $('#btnListado').bind('click', function () { IMPRESION_PROCESOS('#btnListado', 'ListadoFirmas'); });

    $('#btnRegresar').bind('click', function () {
        $("#ddatos").show();
        $("#dvista").hide();
        $('#pvista').empty();
        //$('#btnPagare').linkbutton({ disabled: true });
        //$('#btnCheque').linkbutton({ disabled: true });
        //$('#btnListado').linkbutton({ disabled: true });
    });

    $('#btnLimpiar').bind('click', function () { LIMPIAR_SELECCION_GRID('#dgprocesos',true); });
});
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.d[0] == "1") { $.messager.alert('Error', data.d[1], 'error'); }
                        ...
                        $("#ddatos").hide();
                        //$('#btnPagare').linkbutton({ disabled: false });
                        //$('#btnCheque').linkbutton({ disabled: false });
                        //$('#btnListado').linkbutton({ disabled: false });
                        $("#dvista").show();
