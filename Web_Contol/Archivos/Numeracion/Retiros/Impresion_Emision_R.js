...
$(document).ready(function () {
    ...
    $('#btnListadoEmitidos').bind('click', function () {
        //IMPRESION_PROCESOS('#btnListadoEmitidos', 'ListadoEmitidos_R', 0);
    ...
    $('#btnImprimir').bind('click', function () {       
        $("#win").window('close');        
        if (boton == 'btnPagare') { IMPRESION_PROCESOS('#btnImprimir', 'Finiquito_R', 0); }
        if (boton == 'btnCheque') { IMPRESION_PROCESOS('#btnImprimir', 'ChequeStd_R', 1); }
        if (boton == 'btnListadoFirmas') { IMPRESION_PROCESOS('#btnImprimir', 'ListadoFirmas_R', 1); }
        if (boton == 'btnListadoEmitidos') { IMPRESION_PROCESOS('#btnImprimir', 'ListadoEmitidos_R', 0); }
    });
        ...
        if (checkedRows.length == 0) { $.messager.alert('Error', 'Falta seleccionar por lo menos un proceso', 'error'); return 0; }
            ...
            if (($('#btnRangos').linkbutton('options').selected == true) && $('#txtinicial').numberbox('getValue') == "") { $.messager.alert('Error', 'Falta el cheque inicial', 'error'); return 0; }
                ...
                if (($('#btnRangos').linkbutton('options').selected == true) && $('#txtfinal').numberbox('getValue') == "") { $.messager.alert('Error', 'Falta el cheque final', 'error'); return 0; }
                    ...
                    parametros.Proceso = "Salida_Emisiones@dtRetiros";
                    parametros.Modulo = "R";
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.d[0] == "1") { $.messager.alert('Error', data.d[1], 'error'); }
                                ...
                                NomRep = data.d[2];
                                IR_PAGINA("../Vista_Previa.aspx", "Mod=R&Pag=Impresion_Emision_R&NomRep=" + NomRep);
                            }
                        },
                        error: function (er) {
                            $('#loading').hide();
