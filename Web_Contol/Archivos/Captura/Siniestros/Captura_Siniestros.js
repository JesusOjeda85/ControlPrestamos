...
$(document).ready(function () {
    ...
    $('#btnGuardar').bind('click', function () { GUARDAR_CAPTURA_SINIESTROS('#btnGuardar'); });
...
function LISTAR_CAUSASMUERTE() {
    
    $.ajax({
        type: "POST",
        url: '../Fun_Captura.aspx/LISTAR_CAUSASMUERTE',        
        dataType: "json",
        async: true,
        cache: false,
        contentType: "application/json; charset=utf-8",
...
function LISTAR_CONCEPTOSSINIESTROS() {

    $.ajax({
        type: "POST",
        url: '../Fun_Captura.aspx/LISTAR_CONCEPTOSSINIESTROS',
        dataType: "json",
        async: true,
        cache: false,
        contentType: "application/json; charset=utf-8",
...
function LIMPIAR_CAPTURA() {
    ...
    $('#txtbeneficiario').textbox('setValue', '');
    $('#cbocausamuerte').combobox('setValue', 'x');
    $('#cboconcepto').combobox('setValue', 'x');
...
function CARGAR_EMPLEADOS(btnobj, filtro, cargar) {
    if ($(btnobj).linkbutton('options').disabled) { return false; }
        ...
        if (cargar == "Si") {
            ...
            $('#dgdatos').datagrid({
                url: '../listar_RetirosSiniestros.aspx?busqueda=' + filtro + "&modulo=S",
                pagination: false,
                enableFilter: true,
                rownumbers: true,
                singleSelect: true,
                ...
                onClickRow: function () {
                    ...
                    $('#txtmaterno').textbox('setValue', row.Naterno);
                    $('#txtnombres').textbox('setValue', row.Nombre);

                    if (row.id != 0) {
                        $('#txtbeneficiario').textbox('setValue', row.Beneficiario);
                        ...
                        $('#cbocausamuerte').combobox('setValue', row.fkCausaMuerte);
                        $('#cboconcepto').combobox('setValue', row.fkConceptoSiniestro);
...
function GUARDAR_CAPTURA_SINIESTROS(btnobj) {
    if ($(btnobj).linkbutton('options').disabled) { return false; }
    else {
        if ($('#cbozonapago').combobox('getValue') == "x") { $.messager.alert('Error', "Falta Seleccionar la Zona de Pago", 'error'); }
        else
            if ($('#cbocausamuerte').combobox('getValue') == "x") { $.messager.alert('Error', "Falta Seleccionar la Causa de Muerte", 'error'); }
        else
                if ($('#cboconcepto').combobox('getValue') == "") { $.messager.alert('Error', "Falta Seleccionar el Concepto", 'error'); }
                else
                    if ($('#txtnombres').textbox('getValue') == "") { $.messager.alert('Error', "Falta el Nombre", 'error'); }
                    else
                        if ($('#txtrfc').textbox('getValue') == "") { $.messager.alert('Error', "Falta el Rfc", 'error'); }
                        else
                            if ($('#txtbeneficiario').textbox('getValue') == "") { $.messager.alert('Error', "Falta el Beneficiario", 'error'); }
                            else
                    ...
                    if (($('#txtcuenta').numberbox('getValue') == "") && (parseInt($('#cbotipopago').combobox('getValue')) == 2)) { $.messager.alert('Error', "Falta la Cuenta Bancaria", 'error'); }                                         
                        ...
                        var data = {
                            Obj: {
                                ...
                                Beneficiario: $('#txtbeneficiario').textbox('getValue'),
                                FkCausaMuerte: parseInt($('#cbocausamuerte').combobox('getValue')),
                                FkConceptoSiniestro: parseInt($('#cboconcepto').combobox('getValue')),
                                ImporteBruro: $('#txtimpbruto').numberbox('getValue'),
                            ...
                            $.ajax({
                                type: "POST",
                                url: "../Fun_Captura.aspx/GUARDAR_CAPTURA_SINIESTROS",
                                data: JSON.stringify(data),
                                async: true,
                                cache: false,
                                dataType: "json",
