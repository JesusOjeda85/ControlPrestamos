$(document).ready(function () {
    ...
    $('#btnBempleado').bind('click', function () { CARGAR_EMPLEADOS('#btnBuscar', $('#txtvalor').textbox('getValue'),"No"); });
    ...
    if ($(btnobj).linkbutton('options').disabled) { return false; }
        ...
        if (carga == "Si") {
            $('#dgempleados').datagrid({
                url: "../Consulta/Buscar_Empleados.aspx?modulo=P&busqueda=" + filtro,
                pagination: false,
                enableFilter: false,
                rownumbers: true,
                singleSelect: true,
...
function GUARDAR_CAPTURA(btnobj) {
    if ($(btnobj).linkbutton('options').disabled) { return false; }
        ...
        if ($('#txtadelanto').textbox('getValue') == "") { $.messager.alert('Error', "Faltan el importe adelanto a guardar", 'error'); }
        else
        if ($('#txtobservaciones').textbox('getValue') == "") { $.messager.alert('Error', "Faltan las observaciones", 'error'); }
        else
            if ($('#dbfecCaptura').datebox('getValue') == "") { $.messager.alert('Error', "Faltan la fecha del adelanto", 'error'); }
            else
            var row = $('#dgempleados').datagrid('getSelected');
        var data = {
            Obj: {
                Id: row.Id,
                Importe: $('#txtadelanto').textbox('getValue'),
                FecCaptura: $('#dbfecCaptura').datebox('getValue'),
                Observaciones: $('#txtobservaciones').textbox('getValue').toUpperCase(),
            }
        }
        $.ajax({
            type: "POST",
            url: "Fun_PagosExternos.aspx/GUARDAR_PAGOS_EXTERNOS",
