...
$(document).ready(function () {
    ...
    $('#btnImpresionRevision').bind('click', function () {
        IMPRESION_PROCESOS('#btnImpresionRevision', 'RevisionCaptura_R');      
    });
...
function IMPRESION_PROCESOS(btnobj, reporte) {
    if ($(btnobj).linkbutton('options').disabled) { return false; }
        ...
        if (checkedRows.length == 0) { $.messager.alert('Error', 'Falta seleccionar por lo menos un proceso', 'error'); return 0; }
                    ...
                    parametros.Proceso = "Revision_Captura@dtRetiros";
                    parametros.Modulo = "R";
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.d[0] == "1") { $.messager.alert('Error', data.d[1], 'error'); }
                                ...
                                NomRep = data.d[2];
                                IR_PAGINA("../Captura_Vista_Previa.aspx", "Mod=R&Pag=Impresion_Revision_R&NomRep=" + NomRep);
                            }
                        },
                        error: function (er) {
                            $('#loading').hide();
