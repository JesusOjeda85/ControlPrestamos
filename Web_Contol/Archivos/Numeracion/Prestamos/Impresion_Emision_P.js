...
$(document).ready(function () {
    ...
    $('#btnPagare').bind('click', function () {
        //IMPRESION_PROCESOS('#btnPagare', 'Pagare', 0);
        boton = 'btnPagare';
        windows_porcentaje("#win", 25, 35, false, false, false, "Pagaré");       
    });
    ...
    $('#btnListadoFirmas').bind('click', function () {
        //IMPRESION_PROCESOS('#btnListadoFirmas', 'ListadoFirmas', 1);
    ...
    $('#btnListadoEmitidos').bind('click', function () {
        //IMPRESION_PROCESOS('#btnListadoEmitidos', 'ListadoEmitidos', 0);
    ...
    $('#btnImprimir').bind('click', function () {
        $("#win").window('close');
        if (boton == 'btnPagare') { IMPRESION_PROCESOS('#btnImprimir', 'Pagare', 0); }
        if (boton == 'btnCheque') { IMPRESION_PROCESOS('#btnImprimir', 'ChequeStd_P', 1); }
        if (boton == 'btnListadoFirmas') { IMPRESION_PROCESOS('#btnImprimir', 'ListadoFirmas_P', 1); }
        if (boton == 'btnListadoEmitidos') { IMPRESION_PROCESOS('#btnImprimir', 'ListadoEmitidos_P', 0); }
    });
        ...
        if (checkedRows.length == 0) { $.messager.alert('Error', 'Falta seleccionar por lo menos un proceso', 'error'); return 0; }
            ...
            if (($('#btnRangos').linkbutton('options').selected == true) && $('#txtinicial').numberbox('getValue')=="") { $.messager.alert('Error', 'Falta el cheque inicial', 'error'); return 0; }
                ...
                { 
                    ...
                    parametros.Proceso = "Salida_Emisiones@dtPagare";
                    parametros.Modulo = "P"
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.d[0] == "1") { $.messager.alert('Error', data.d[1], 'error'); }
                                ...
                                NomRep = data.d[2];
                                IR_PAGINA("../Vista_Previa.aspx", "Mod=P&Pag=Impresion_Emision_P&NomRep=" + NomRep);
                            }
                        },
                        error: function (er) {
                            $('#loading').hide();
